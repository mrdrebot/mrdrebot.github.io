...
let freeCount = 0;
let busyCount = 0;
let freePlaces = document.querySelector(".free-places");
let busyPlaces = document.querySelector(".busy-places");
...
const countPlaces = () => {
    ...
    freePlaces.innerHTML = `Свободных мест = ${freeCount}`;
...
for(let i = 0; i < parkingPlacesArr.length; i++) {
    ...
    // parkingPlacesArr[i].innerHTML = `<div>ID = ${parkingArr[i].id}<br><div class="id-${i + 1}-status">${(parkingArr[i].occupied) ? "Busy" : "Free"}</div><br>Place taken ${parkingArr[i].time}`;
...
parking.addEventListener("click", (clickEvent) => {
    ...
    let inputTime = document.querySelector("#modal-window");
